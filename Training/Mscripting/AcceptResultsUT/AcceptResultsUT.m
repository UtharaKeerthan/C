...
root = bdroot;
SigBuildr = strcat(root,'/Inputs/Signal Builder');
...
%%Filtering only the expected signals
Index = strfind(signames,'_Expected');
signames = signames(find(not(cellfun('isempty', Index))));
...
Index = strfind(TSnames,'_Diff');%%Removing the Diff Timeseries
TSnames = TSnames(find((cellfun('isempty', Index))));
...
htext2     = uicontrol('Style','text','String','EXPECTED SIGNAL :','HorizontalAlignment','center','parent',grid_one);
...
function hpopup1_Callback(source,eventdata)
    try
      ...
      signalbuilder(SigBuildr, 'activegroup', val);
    catch exception
      errordlg(exception.message,'Error');%%Display error msg if any
    end
end
